...
def order_books_by_year() -> str:
    all_books = Book.objects.order_by("publication_year", "title")
...
def delete_review_by_id(reviews_id: int) -> str:
    searched_review = Review.objects.get(id=reviews_id)
    searched_review.delete()

    return f"Review by {searched_review.reviewer_name} was deleted"


def filter_authors_by_nationalities(searched_nationality: str) -> str:
    searched_authors = ((Author.objects
    ...
    for author in searched_authors:
        result.append(
            author.biography if author.biography else f"{author.first_name} {author.last_name}"
        )

    return "\n".join(result)


def filter_authors_by_birth_year(year_1: int, year_2: int) -> str:
    searched_authors = (Author.objects
                        .filter(birth_date__year__range=(year_1, year_2))
                        .order_by("-birth_date"))
...
def change_reviewer_name(searched_name: str, new_name: str) -> QuerySet:
    Review.objects.filter(reviewer_name=searched_name).update(reviewer_name=new_name)
